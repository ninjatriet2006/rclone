...
pub fn draw_input_paste_rename_popup(frame: &mut Frame, input_buffer: &str, cursor_idx: usize) {
    let size = frame.size();
    ...
    let text = vec![
        Line::from(translate("exp_paste_rename_prompt")),
        Line::from(""),
        Line::from(spans),
        Line::from(""),
    ...
    let block = Block::default()
        .title(Span::styled(
            translate("exp_paste_rename_title"),
            Style::default()
                .fg(Color::Cyan)
