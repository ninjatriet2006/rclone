use crate::app::{App, AppEvent, Screen, DeleteTarget, MultiScanState};
use crate::functions::*;
...
pub async fn handle_explorer_keys(
    app: &mut App,
    key: KeyEvent,
        tx: tokio::sync::mpsc::UnboundedSender<AppEvent>,
) {
        let popup = app.explorer_state.popup.clone();
        if popup != ExplorerPopup::None {
            match popup {

                ExplorerPopup::InputRename {
                    old_name,
                    mut input_buffer,
                    is_dir,
                } => {
                ...
                ExplorerPopup::InputNewFolder { mut input_buffer } => {
                    let mut cursor = app.explorer_state.edit_cursor_idx;
                    if handle_input_key(&key, &mut input_buffer, &mut cursor) {
                        app.explorer_state.edit_cursor_idx = cursor;
                ...
                ExplorerPopup::CopyProgress { job_id, .. }
                | ExplorerPopup::MoveProgress { job_id, .. } => {
                    if (key.code == KeyCode::Char('c') && key.modifiers.contains(crossterm::event::KeyModifiers::CONTROL))
                        || key.code == KeyCode::Esc
                    {
                ...
                ExplorerPopup::ViewFile { file_name, content, mut scroll_offset } => {
                    match key.code {
                        KeyCode::Esc => {
                            app.explorer_state.popup = ExplorerPopup::None;
                        }
                ...
                ExplorerPopup::ChecksumTypeSelect { mut selected_idx } => {
                ...
                ExplorerPopup::CryptdecodeForm { mut remote_input, mut encrypted_input, mut is_remote_focused, output_result } => {
                    let mut cursor = app.explorer_state.edit_cursor_idx;
                ...
                ExplorerPopup::DecompressModeSelect { archive_path, mut selected_idx } => {
                    match key.code {
                        KeyCode::Esc => {
                            app.explorer_state.popup = ExplorerPopup::None;
                        }
                ...
                ExplorerPopup::DecompressPathInput { archive_path, mut selected_idx } => {
                    match key.code {
                        KeyCode::Esc => {
                            app.explorer_state.popup = ExplorerPopup::None;
                        }
                ...
                ExplorerPopup::DecompressPathManualInput { archive_path, mut input_buffer } => {
                    let mut cursor = app.explorer_state.edit_cursor_idx;
                    if handle_input_key(&key, &mut input_buffer, &mut cursor) {
                        app.explorer_state.edit_cursor_idx = cursor;
                ...
                ExplorerPopup::TuiExplorerSelector {
                    archive_path,
                    mut remote,
                    mut path,
                    items,
                ...
                ExplorerPopup::InputPasteRename { mut input_buffer } => {
                    let mut cursor = app.explorer_state.edit_cursor_idx;
                    if handle_input_key(&key, &mut input_buffer, &mut cursor) {
                        app.explorer_state.edit_cursor_idx = cursor;
                ...
                ExplorerPopup::InputSharedLink { mut input_buffer } => {
                    let mut cursor = app.explorer_state.edit_cursor_idx;
                    if handle_input_key(&key, &mut input_buffer, &mut cursor) {
                        app.explorer_state.edit_cursor_idx = cursor;
                            ...
                            KeyCode::Enter => {
                                ...
                                if !link.is_empty() {
                                    ...
                                    if drive_remotes.is_empty() {
                                        app.explorer_state.notification = Some(("CẢNH BÁO".to_string(), "Không tìm thấy remote Google Drive nào được cấu hình trong hệ thống làm base credentials!".to_string()));
                                        app.explorer_state.popup = ExplorerPopup::None;
                                    } else {
                ...
                ExplorerPopup::SelectBaseRemote {
                    remotes,
                    mut selected_idx,
                    folder_id,
                } => match key.code {
                ...
                ExplorerPopup::DedupeModeSelect {
                    mut by_hash,
                    mut selected_idx,
                } => match key.code {
                    KeyCode::Esc => {
                ...
                ExplorerPopup::MergeSimilarDestinationSelect { folders, mut selected_idx } => match key.code {
                    KeyCode::Esc => {
                        app.explorer_state.popup = ExplorerPopup::None;
                    }
                    KeyCode::Up => {
                ...
                ExplorerPopup::MergeSimilarScanning { .. } => match key.code {
                    KeyCode::Esc => {
                        app.explorer_state.popup = ExplorerPopup::None;
                    }
                    _ => {}
        ...
        match key.code {
            ...
            KeyCode::Tab => {
                app.explorer_state.toggle_pane();
            }
            KeyCode::Up => {
                app.explorer_state.get_active_pane_mut().prev();
            }
            KeyCode::Down => {
                app.explorer_state.get_active_pane_mut().next();
            }
            KeyCode::Enter => {
                ...
                if !active_pane.items.is_empty() {
                    ...
                    if selected.name == ".." {
                        ...
                        if is_text && selected.size <= 2_000_000 {
                            ...
                            app.explorer_state.popup = ExplorerPopup::SpecialActionMessage {
            ...
            KeyCode::Char('r') | KeyCode::Char('R') if key.modifiers.contains(KeyModifiers::ALT) || (cfg!(target_os = "macos") && key.modifiers.contains(KeyModifiers::CONTROL)) => {
                ...
                remotes.extend(app.connection_state.remotes.clone());
                remotes.push(translate("exp_add_shared_link_option"));
                app.explorer_state.popup = ExplorerPopup::SelectRemote {
                    remotes,
                    selected_idx: 0,
                };
            ...
            KeyCode::Char('n') | KeyCode::Char('N') if key.modifiers.contains(KeyModifiers::ALT) || (cfg!(target_os = "macos") && key.modifiers.contains(KeyModifiers::CONTROL)) => {
                app.explorer_state.edit_cursor_idx = 0;
                app.explorer_state.popup = ExplorerPopup::InputNewFolder {
                    input_buffer: String::new(),
                };
            }
            KeyCode::Char('y') | KeyCode::Char('Y') if key.modifiers.contains(KeyModifiers::ALT) || (cfg!(target_os = "macos") && key.modifiers.contains(KeyModifiers::CONTROL)) => {
                ...
                if let (Some(name), Some(is_dir)) = (name, is_dir) {
                    app.explorer_state.edit_cursor_idx = name.chars().count();
                    app.explorer_state.popup = ExplorerPopup::InputRename {
                        old_name: name.clone(),
                        input_buffer: name,
                        is_dir,
            ...
            KeyCode::Delete => {
                ...
                if !pane.items.is_empty() {
                    ...
                    if !pane.selected_names.is_empty() {
                        ...
                        app.delete_confirm = Some(DeleteTarget::FileExplorerMultiple(names));
                    } else {
                        let item_name = pane.items[pane.selected_idx].name.clone();
                        if item_name != ".." {
                            app.delete_confirm = Some(DeleteTarget::FileExplorer(item_name));
                        }
                    }
                }
            }
            ...
            KeyCode::Char('c') | KeyCode::Char('C') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                ...
                if !pane.items.is_empty() {
                    if !pane.selected_names.is_empty() {
                        ...
                        let items: Vec<ClipboardItem> = pane.items.iter()
                            ...
                            .collect();
                        app.explorer_state.clipboard_items = Some(items);
                        app.explorer_state.clipboard = None;
                    } else {
                        // Single select
                        let item = &pane.items[pane.selected_idx];
                        if item.name != ".." {
                            app.explorer_state.clipboard = Some(ClipboardItem {
                                remote: pane.remote.clone(),
                                path: pane.path.clone(),
                                name: item.name.clone(),
                                is_dir: item.is_dir,
                            });
                            app.explorer_state.clipboard_items = None;
                        }
                    }
                }
            }
            KeyCode::Char('o') | KeyCode::Char('O') if key.modifiers.contains(KeyModifiers::ALT) || (cfg!(target_os = "macos") && key.modifiers.contains(KeyModifiers::CONTROL)) => {
                ...
                if !pane.items.is_empty() {
                    ...
                    if selected.name != ".." {
                        app.explorer_state.popup = ExplorerPopup::SpecialActionsMenu {
                            selected_idx: 0,
                        };
                    }
                }
            ...
            KeyCode::Char('v') | KeyCode::Char('V') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                ...
                if let Some(ref items) = app.explorer_state.clipboard_items {
                    if !items.is_empty() {
                        let dest_pane = app.explorer_state.get_active_pane();
                ...
                } else if let Some(ref clipboard_item) = app.explorer_state.clipboard {
                    app.explorer_state.edit_cursor_idx = clipboard_item.name.chars().count();
                    app.explorer_state.popup = ExplorerPopup::InputPasteRename {
            ...
            KeyCode::Char('x') | KeyCode::Char('X') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                ...
                let src_pane = app.explorer_state.get_active_pane();
                if !src_pane.items.is_empty() {
                    if !src_pane.selected_names.is_empty() {
                        ...
                        let dest_pane = match app.explorer_state.active_pane {
                            ActivePane::Left => &app.explorer_state.right_pane,
                            ActivePane::Right => &app.explorer_state.left_pane,
                        };
                        let dest_remote = dest_pane.remote.clone();
                        let dest_path = dest_pane.path.clone();
                        let pane_type = app.explorer_state.active_pane.clone();
                        ...
                        app.explorer_state.get_active_pane_mut().selected_names.clear();
                        ...
                        app.check_features_and_execute("move", src, dest, is_dir, false, tx.clone());
                    }
                }
            }
            KeyCode::Char('t') | KeyCode::Char('T') if key.modifiers.contains(KeyModifiers::ALT) || (cfg!(target_os = "macos") && key.modifiers.contains(KeyModifiers::CONTROL)) => {
                // Alt+T: Đồng bộ
                let src_pane = match app.explorer_state.active_pane {
                    ActivePane::Left => &app.explorer_state.left_pane,
                    ActivePane::Right => &app.explorer_state.right_pane,
                };
                let dest_pane = match app.explorer_state.active_pane {
                    ActivePane::Left => &app.explorer_state.right_pane,
                    ActivePane::Right => &app.explorer_state.left_pane,
